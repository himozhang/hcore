...
import com.ideal.framework.constants.FileConstants;
...
public class ApacheFileUtils extends org.apache.commons.io.FileUtils {
	...
	private static Logger log = LoggerFactory.getLogger(ApacheFileUtils.class);
	...
	public static boolean copyFile(String srcFileName, String descFileName) {
		return ApacheFileUtils.copyFileCover(srcFileName, descFileName, false);
	}
	...
	public static boolean copyFileCover(String srcFileName,
		...
		if (descFile.exists()) {
			...
			if (coverlay) {
				...
				if (!ApacheFileUtils.delFile(descFileName)) {
	...
	public static boolean copyDirectory(String srcDirName, String descDirName) {
		return ApacheFileUtils.copyDirectoryCover(srcDirName, descDirName,
				false);
	}
	...
	public static boolean copyDirectoryCover(String srcDirName,
		...
		if (descDir.exists()) {
			if (coverlay) {
				...
				if (!ApacheFileUtils.delFile(descDirNames)) {
		...
		for (int i = 0; i < files.length; i++) {
			// 如果是一个单个文件，则直接复制
			if (files[i].isFile()) {
				flag = ApacheFileUtils.copyFile(files[i].getAbsolutePath(),
			...
			if (files[i].isDirectory()) {
				flag = ApacheFileUtils.copyDirectory(files[i]
	...
	public static boolean delFile(String fileName) {
		...
		if (!file.exists()) {
			...
			if (file.isFile()) {
				return ApacheFileUtils.deleteFile(fileName);
			} else {
				return ApacheFileUtils.deleteDirectory(fileName);
			}
		}
	}
	...
	public static boolean deleteDirectory(String dirName) {
		...
		for (int i = 0; i < files.length; i++) {
			// 删除子文件
			if (files[i].isFile()) {
				flag = ApacheFileUtils.deleteFile(files[i].getAbsolutePath());
				// 如果删除文件失败，则退出循环
				if (!flag) {
					break;
				}
			...
			// 删除子目录
				...
				flag = ApacheFileUtils.deleteDirectory(files[i]
						.getAbsolutePath());
				// 如果删除子目录失败，则退出循环
				if (!flag) {
					break;
	...
	public static void zipFiles(String srcDirName, String fileName,
		...
		try {
			...
			if ("*".equals(fileName) || "".equals(fileName)) {
				ApacheFileUtils.zipDirectoryToZipFile(dirPath, fileDir, zouts);
			} else {
				File file = new File(fileDir, fileName);
				if (file.isFile()) {
					ApacheFileUtils.zipFilesToZipFile(dirPath, file, zouts);
				} else {
					ApacheFileUtils
	...
	public static boolean unZipFiles(String zipFileName, String descFileName) {
        ...
        try {
			...
			byte[] buf = new byte[FileConstants.BUFFER_SIZE];
			int readByte = 0;
			// 获取ZIP文件里所有的entry
		...
		if (fileDir.isDirectory()) {
			...
			for (int i = 0; i < files.length; i++) {
				if (files[i].isFile()) {
					...
					// 如果是目录，则递归调用
					ApacheFileUtils.zipDirectoryToZipFile(dirPath, files[i],
							zouts);
				}
			}
